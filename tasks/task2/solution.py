...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.first_moments = self._create_weight_copy()
        self.second_moments = self._create_weight_copy()
        

        # Full SWAG
        # TODO(2): create attributes for SWAG-full
        #  Hint: check collections.deque
        self.max_rank = deviation_matrix_max_rank
        self.cov_sq = collections.deque(maxlen=self.max_rank)
        self.cov_sq.append(self._create_weight_copy())
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            self.first_moments[name] = ((self.epoch_iter * self.first_moments[name]) + param) / (self.epoch_iter + 1)
            self.second_moments[name] = ((self.epoch_iter * self.second_moments[name]*2) + param*2) / (self.epoch_iter + 1)
            # self.first_moments[name] += (param / (self.swag_epochs // self.swag_update_freq))
            # self.second_moments[name] += (param**2 / (self.swag_epochs // self.swag_update_freq))

        # Full SWAG
        if self.inference_mode == InferenceMode.SWAG_FULL:
            # TODO(2): update full SWAG attributes for weight name using current_params and param
            if len(self.cov_sq) == self.max_rank:
                self.cov_sq.popleft()
            self.cov_sq.append(self._create_weight_copy())
            for name, param in current_params.items():

                self.cov_sq[-1][name] = param - self.first_moments[name]
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                # TODO(1): Implement periodic SWAG updates using the attributes defined in init
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        self._prediction_threshold = 1.85 / 3.0
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.first_moments[name]
            # we check for the possibility of havin negative entries in the variance by setting a lower limit of self.var_clamp
            current_std = torch.sqrt(torch.clamp(self.second_moments[name] - current_mean**2, self.var_clamp))
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                current_cov = torch.zeros((self.max_rank, np.prod(param.size())))
                for k in range(self.max_rank):
                    current_cov[k,:] = self.cov_sq[k][name].reshape(-1)

                z_2 = current_cov.new_empty((current_cov.size(0), 1)).normal_()
                
                norm_rescaled = ( 1/ ((self.max_rank - 1) ** 0.5)) * current_cov.t().matmul(z_2).view_as(current_mean)

                sampled_param = current_mean + norm_rescaled
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    and add+store additional attributes in init.
