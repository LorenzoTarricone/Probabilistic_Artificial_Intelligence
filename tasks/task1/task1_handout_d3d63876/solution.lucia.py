...
class Model(object):
    ...
    def __init__(self, kernel = C() * Matern(length_scale=0.05, nu=2.5) + WhiteKernel(noise_level=1e-05)):
        ...
        self.gp = GaussianProcessRegressor(kernel=kernel, alpha=1e-5, random_state=self.rng.integers(0,100))
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray, alpha = 0.5) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        #costs = np.zeros((len(gp_mean), gp_potential_pred.shape[0]))
        #for i in range(len(gp_mean)):
        #    for j in range(gp_potential_pred.shape[0]):
        #        area_id = np.repeat(test_x_AREA[i], gp_samples.shape[0])
        #        costs[i,j] = cost_function(gp_samples[:, i], np.repeat(gp_potential_pred[j, i], gp_samples.shape[0]), area_id)

        #costs = cost_function(np.expand_dims(gp_samples, 1), np.expand_dims(gp_potential_pred, 2), test_x_AREA)
        
        #predictions = gp_potential_pred[np.argmin(costs, axis=1), np.arange(len(gp_mean))]
...
def optimal_kernel(train_y: np.ndarray, train_x_2D: np.ndarray):
    
    kernels = [
    RBF(length_scale=0.05),
    C() * Matern(length_scale=1.0, nu=0.5),
    ...
    marginal_likelihood = np.zeros(len(kernels))
    optimized_kernels = []

    for i, kernel in enumerate(kernels):
        print('Trying kernel')
        model = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=3, alpha=1e-5)
        model.fit(train_x_2D, train_y)
        marginal_likelihood[i] = model.log_marginal_likelihood()
        print(model.kernel_)
        optimized_kernels.append(model.kernel_)
        #optimized_hyperparameters[i] = model.kernel_.theta

    best_kernel = np.array(optimized_kernels)[np.argmin(marginal_likelihood)]
    #print(optimized_hyperparameters[np.argmin(marginal_likelihood)])
...
def main():
    ...
    train_x_v = train_x[train_size:int(1.5*train_size)]
    train_y_n = train_y[:train_size]
    train_y_v = train_y[train_size:int(1.5*train_size)]
    ...
    best_kernel = optimal_kernel(train_y_n, train_x_2D)

    # Fit the model
    print('Fitting model')
    model = Model(kernel=best_kernel)
