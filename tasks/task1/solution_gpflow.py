...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel = gpflow.kernels.RBF(lengthscales=0.01)
        ...
        #self.gp = GaussianProcessRegressor(kernel=self.kernel, n_restarts_optimizer=5, random_state=self.rng.integers(0,100), alpha=0.05)
        self.sparse_gp = None
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray, alpha = 0.4) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gp_mean, gp_std = self.sparse_gp.predict(test_x_2D)
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        #train_x_2D, train_y = subsample(train_x_2D, train_y, 65)
        #self.gp.fit(train_x_2D, train_y)

        ind_points = np.random.rand(self.num_inducing_points, train_x_2D.shape[1])
        self.sparse_gp = gpflow.models.SGPR(train_x_2D, train_y, self.kernel, ind_points)
        self.sparse_gp.likelihood.variance.assign(self.noise_variance)
        gpflow.optimizers.Scipy().minimize(self.sparse_gp.training_loss, self.sparse_gp.trainable_variables)
