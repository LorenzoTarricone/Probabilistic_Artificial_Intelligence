...
class Actor:
    ...
    def get_action_and_log_prob(self, state: torch.Tensor, 
        ...
        if deterministic:
            ...
            z = normal_dist.rsample() + np.random.uniform(-0.5, 0.5)
            action = torch.tanh(z)  # Ensure actions are in range [-2, 2]
            log_prob = normal_dist.log_prob(z).sum(axis=-1)
...
class Agent:
    ...
    def setup_agent(self):
        ...
        self.critic_target = Critic(hidden_size=256, hidden_layers=2, critic_lr=3e-4,
                                    state_dim=self.state_dim, action_dim=self.action_dim,
                                    device=self.device)
    ...
    def train_agent(self):
        ...
        next_actions = self.actor.get_action_and_log_prob(next_states, deterministic = False)[0]
        ...
        target_Q1, target_Q2 = self.critic_target.model1(next_inputs), self.critic_target.model2(next_inputs)
        target_V = torch.min(target_Q1, target_Q2) - self.temperature.get_param().detach() * self.actor.get_action_and_log_prob(next_states, deterministic = False)[1]
        ...
        current_Q1, current_Q2 = self.critic.model1(current_inputs), self.critic.model2(current_inputs)
        ...
        if self.counter % self.critic_target_update_freq == 0:
            self.critic_target_update(self.critic.model1, self.critic_target.model1, tau = self.tau, soft_update = True)
            self.critic_target_update(self.critic.model2, self.critic_target.model2, tau = self.tau, soft_update = True)
